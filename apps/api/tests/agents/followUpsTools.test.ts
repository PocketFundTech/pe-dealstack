...
//
// Ported (2026-08-14) from LangChain tool() objects (`.invoke()`) to plain
// BetaRunnableTool objects (`.run()`) — see the MERGE NOTE resolution in
// dealChatAgent/tools.ts. Both getDealChatTools (streaming) and
// getDealChatToolsLegacy (via toLangChainTool) now share this same
// implementation, so this test covers both barrels' business logic.
...
describe('/follow-ups live-integration tools — userId guard', () => {
  it('get_recent_emails_for_deal returns the no-context message when userId is undefined', async () => {
    ...
    const out = await t.run({ lookback_days: 14, limit: 10 });
  ...
  it('get_upcoming_meetings_for_deal returns the no-context message when userId is undefined', async () => {
    ...
    const out = await t.run({ past_days: 7, future_days: 14, limit: 10 });
    expect(typeof out).toBe('string');
    expect(out).toContain('User context not available');
  });
});
